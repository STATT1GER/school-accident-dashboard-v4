...
def time_profile(df: pd.DataFrame) -> go.Figure:
    ...
    counts["상대사고집중도"] = counts["사고건수"] / maximum * 100
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=counts["사고시간_정리"], y=counts["상대사고집중도"],
        ...
        hovertemplate="%{x}<br><b>상대 사고집중도 %{y:.0f}</b><br>사고 시연 건수 %{customdata[0]:,}건<extra></extra>",
    ))
    fig.update_layout(showlegend=False)
    fig.update_yaxes(title="상대 사고집중도", range=[0, 110])
...
def weekday_time_heatmap(df: pd.DataFrame) -> go.Figure:
    ...
    concentration = pivot / maximum * 100
    fig = go.Figure(go.Heatmap(
        z=concentration.values, x=concentration.columns, y=concentration.index,
        customdata=pivot.values,
        colorscale=[[0, "#F5F5F7"], [0.35, "#BDE0FF"], [1, "#0066CC"]],
        colorbar=dict(title="상대 사고집중도", thickness=10, len=0.75),
        hovertemplate="%{y} · %{x}<br><b>상대 사고집중도 %{z:.0f}</b><br>사고 시연 건수 %{customdata:,}건<extra></extra>",
    ))
